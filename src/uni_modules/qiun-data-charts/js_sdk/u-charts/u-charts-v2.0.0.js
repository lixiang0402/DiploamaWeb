    ...
    for (let i = 0; i < opts.yAxis.data.length; i++) {
      ...
      if (yData.disabled !== true) {
        ...
        if (opts.yAxis.showTitle) {
          ...
          if (yAxisWidth.position == 'left') {
            context.fillText(title, tStartLeft - measureText(title, titleFontSize, context) / 2, opts.area[0] - 10 * opts.pix)
          } else {
            context.fillText(title, tStartRight - measureText(title, titleFontSize, context) / 2, opts.area[0] - 10 * opts.pix)
          }
          context.closePath()
          context.stroke()
        }
